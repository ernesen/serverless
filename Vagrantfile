...
servers=[
  {
    :hostname => "serverless." + "#{DOMAIN}",
    :ip => "#{BRIDGE_NET}" + "157",
    :ram => "#{RAM}",
	:install_docker => "./artefacts/scripts/install_docker.sh", 
	:config_serverless => "./artefacts/scripts/config_serverless.sh",
	:source =>  "./artefacts/.",
        ...
        config.vm.define machine[:hostname] do |node|
            ...
            node.vm.provider "virtualbox" do |vb|
                ...
                if (!machine[:install_docker].nil?)
                  ...
                  if File.exist?(machine[:config_serverless])
					node.vm.provision :file, source: machine[:source] , destination: machine[:destination]
      			    node.vm.provision :shell, privileged: false, path: machine[:config_serverless]
                  end
                end
            end
        end
